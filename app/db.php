<?php
	...
	/*
		...
		the DATABASE constant in definitions.php to the desired driver name, for example 'mssql'.
	...
	function db_connect($host = NULL, $username = NULL, $passwd = NULL, $dbname = NULL, $port = NULL, $socket = NULL) {
		switch(DATABASE) {
			case 'mysql':
				...
				if($dbname) { if(!mysql_select_db($dbname, link_identifier: $link)) return false; }
				return $link;
			case 'mysqli':
